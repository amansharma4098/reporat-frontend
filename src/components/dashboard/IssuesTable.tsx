...
export default function IssuesTable({ issues }: { issues: Issue[] }) {
  ...
  return (
    <>
            ...
            {issues.map((issue) => (
              <tr
                ...
                className={`border-b border-slate-100 hover:bg-slate-50 transition-colors cursor-pointer ${
                  selectedIssue?.id === issue.id ? "bg-emerald-50/50" : ""
                }`}
              >
      ...
      {/* Issue detail panel */}
      {selectedIssue && (
        <div className="mt-4 bg-white border border-slate-200 rounded-xl shadow-lg p-6">
          ...
          {selectedIssue.raw_output && (
            <details className="mt-4">
              ...
              <pre className="mt-2 bg-slate-900 text-slate-300 rounded-lg p-4 text-xs overflow-x-auto max-h-64 overflow-y-auto">
                {selectedIssue.raw_output}
              </pre>
            </details>
          )}

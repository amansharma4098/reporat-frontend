...
export default function IssuesTable({ issues }: { issues: Issue[] }) {
  ...
  if (!issues?.length) {
    return (
      <div className="bg-white border border-gray-200 rounded-xl p-8 text-center">
        <AlertTriangle size={24} className="mx-auto text-gray-300 mb-3" />
  ...
  return (
    <>
            ...
            {issues.map((issue, idx) => (
              <tr
                ...
                className={`border-b border-gray-100 hover:bg-gray-50 transition-colors cursor-pointer text-sm ${
                  selectedIssue?.id === issue.id ? "bg-emerald-50/50" : idx % 2 === 1 ? "bg-gray-50/30" : ""
                }`}
              >
      ...
      {/* Issue detail panel */}
      {selectedIssue && (
        <div className="mt-4 border-l-4 border-emerald-500 bg-emerald-50/30 rounded-lg p-5 animate-fade-in">
          ...
          {selectedIssue.raw_output && (
            <details className="mt-4">
              ...
              <pre className="mt-2 bg-gray-900 text-gray-300 rounded-lg p-4 text-xs overflow-x-auto max-h-64 overflow-y-auto font-mono">
                {selectedIssue.raw_output}
              </pre>
            </details>
          )}
